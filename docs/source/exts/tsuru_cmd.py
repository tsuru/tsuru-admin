...
def main():
    parts_regex = re.compile(r'tsuru-admin version.*Usage: (.*?)\n+(.*)', re.DOTALL)
    topic_regex = re.compile(r'tsuru-admin version.*?\n+(.*)\n\n.*?\n\n  ', re.DOTALL)

    result = check_output("tsuru-admin | egrep \"^[  ]\" | awk -F ' ' '{print $1}'", shell=True)
    cmds = result.split('\n')
    final_result = {}
    for cmd in cmds:
        if not cmd:
            continue
        result = check_output('tsuru-admin help {}'.format(cmd), shell=True)
